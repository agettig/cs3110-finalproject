  ...
  if gameover gamestate.players then
    ...
    let new_player : player * cards option =
      ...
      | HouseTile _ ->
          ...
          (add_card chosen_house pay_player, Some chosen_house)
      | TakeTile _ -> (pay_player, None) (* not implemented in ms1*)
      | ActionTile _ -> (pay_player, None)
      | SpinToWinTile _ ->
...
let payraise_tiles = (39, 71, 99, 114)
